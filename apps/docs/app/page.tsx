...
function Icon({ name, size = 15 }: { name: "cart" | "heart" | "search" | "swap" | "pin"; size?: number }) {
  const common = { width: size, height: size, viewBox: "0 0 24 24", fill: "none", "aria-hidden": true } as const;
  if (name === "cart") return <svg {...common}><path d="M3.5 4.5h2l1.55 9.02a2 2 0 0 0 1.97 1.66h7.88a2 2 0 0 0 1.94-1.52L20.4 8H7" stroke="currentColor" strokeWidth="1.65" strokeLinecap="round" strokeLinejoin="round"/><circle cx="9.5" cy="19" r="1.25" stroke="currentColor" strokeWidth="1.65"/><circle cx="17.5" cy="19" r="1.25" stroke="currentColor" strokeWidth="1.65"/></svg>;
  if (name === "heart") return <svg {...common}><path d="M20.8 5.8c-1.7-1.8-4.6-1.8-6.3 0L12 8.35 9.5 5.8a4.45 4.45 0 0 0-6.3 6.3L12 21l8.8-8.9a4.45 4.45 0 0 0 0-6.3Z" stroke="currentColor" strokeWidth="1.55" strokeLinecap="round" strokeLinejoin="round"/></svg>;
  if (name === "search") return <svg {...common}><circle cx="10.8" cy="10.8" r="6.3" stroke="currentColor" strokeWidth="1.55"/><path d="m15.5 15.5 4 4" stroke="currentColor" strokeWidth="1.55" strokeLinecap="round"/></svg>;
  if (name === "swap") return <svg {...common}><path d="M7 7h11m0 0-3-3m3 3-3 3M17 17H6m0 0 3 3m-3-3 3-3" stroke="currentColor" strokeWidth="1.55" strokeLinecap="round" strokeLinejoin="round"/></svg>;
  return <svg {...common}><path d="M12 21s6-5.2 6-11a6 6 0 1 0-12 0c0 5.8 6 11 6 11Z" stroke="currentColor" strokeWidth="1.55"/><circle cx="12" cy="10" r="2.1" stroke="currentColor" strokeWidth="1.55"/></svg>;
}
  ...
  return (
    <main className="review-board">
      <section className="board-column board-column--left">
        ...
        <Card className="spec-card button-spec">
          ...
          <div className="button-grid">
            <Button leadingIcon={<Icon name="cart" size={13} />}>Agregar al carrito</Button>
          ...
          <div className="button-grid button-grid--secondary">
            <Button variant="secondary" leadingIcon={<Icon name="heart" size={13} />}>Favorito</Button>
            <Button disabled leadingIcon={<Icon name="cart" size={13} />}>Agregar al carrito</Button>
          </div>
          <div className="states-row"><div><small>Focus</small><Button className="is-focus" leadingIcon={<Icon name="cart" size={13} />}>Agregar al carrito</Button></div><div><small>Loading</small><Button loading>Cargando…</Button></div><div className="icon-buttons"><small>Icon button</small><div><button aria-label="Favorito"><Icon name="heart" size={14} /></button><button aria-label="Comparar"><Icon name="swap" size={14} /></button><button aria-label="Carrito"><Icon name="cart" size={14} /><b>4</b></button></div></div></div>
        </Card>
        ...
        <Card className="spec-card">
          ...
          <div className="search-grid"><div><small>Search · Default</small><Search placeholder="Buscar plantas, macetas, sustratos..." endAction={<Icon name="search" size={14} />} /></div><div><small>Search · Focus</small><Search className="demo-focus-wrap" placeholder="Buscar plantas, macetas, sustratos..." endAction={<Icon name="search" size={14} />} /></div></div>
        </Card>
      ...
      <section className="board-column board-column--middle">
        ...
        <Card className="spec-card product-variants">
          ...
          <div className="variant-grid">
            <ProductCard imageSrc={plant()} imageAlt="Monstera" badge="Fácil cuidado" title="Monstera Deliciosa" description="Ideal para interiores luminosos" price="$19.900" stock="Stock: 24 unidades" favoriteAction={<button className="heart" aria-label="Favorito"><Icon name="heart" size={14} /></button>} />
            <ProductCard className="force-hover" imageSrc={plant("#42a567")} imageAlt="Monstera" badge="Fácil cuidado" title="Monstera Deliciosa" description="Ideal para interiores luminosos" price="$19.900" stock="Stock: 24 unidades" favoriteAction={<button className="heart" aria-label="Favorito"><Icon name="heart" size={14} /></button>} />
            <ProductCard className="featured-card" imageSrc={plant("#2a914f")} imageAlt="Monstera" badge="✿ Más vendido" title="Monstera Deliciosa" description="Ideal para interiores luminosos" price="$19.900" stock="Stock: 24 unidades" favoriteAction={<button className="heart" aria-label="Favorito"><Icon name="heart" size={14} /></button>} />
          </div>
        </Card>
        ...
        <Card className="spec-card commerce-grid-card">
          ...
          <div className="mini-grid">
            {["Ficus Lyrata","Lavanda","Sansevieria Laurentii","Poto Dorado"].map((title,index)=><ProductCard key={title} imageSrc={plant(["#2f8e50","#7c65b8","#739c38","#49a56d"][index])} imageAlt={title} badge={["Fácil cuidado","Aromática","Purifica el aire","Colgante"][index]} title={title} description="Hojas grandes y decorativas" price={["$24.900","$8.500","$9.800","$7.900"][index]} stock={["Stock: 18","Stock: 30","Stock: 12","Stock: 18"][index]} favoriteAction={<button className="mini-heart" aria-label="Favorito"><Icon name="heart" size={11} /></button>} />)}
          </div>
        </Card>
      </section>
      ...
      <section className="bottom-navigation">
        <Header logoSrc="/verdea-lockup-horizontal.svg" search={<Search placeholder="Buscar plantas, macetas, sustratos..." endAction={<Icon name="search" size={14} />} />} actions={<><Button size="sm" leadingIcon={<Icon name="pin" size={13} />}>VERDEA Centro</Button><span className="header-action"><Icon name="heart" size={14}/>Favoritos</span><span className="header-action"><Icon name="cart" size={14}/>Mi carrito</span><span>Hola, Ana⌄</span></>} />
